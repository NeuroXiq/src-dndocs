...
namespace DNDocs.Job.Web.Services
{
    ...
    {
        Task Handle(BuildNugetOrgProjectModel p);
    }
    ...
    public class DocsBuilderService : IDocsBuilderService
    {
        ...
        public async Task Handle(BuildNugetOrgProjectModel project)
        {
            ...
            try
            {
                ...
                docfxManager.Init(tempFolder.OSFullPath);

                FetchNugetXmlAndDlls(new List<NugetPackage>() { new NugetPackage(project.PackageName, project.PackageVersion) }, docfxManager.OSPathBinDir);
                ...
                appTitle = $"{project.PackageName} {project.PackageVersion} | DNDocs";
                articlesHomepageContent = apiHomepageContent = homepageContent =
                    $"# Welcome on {project.PackageName} {project.PackageVersion} API\r\n" +
                    $"#### Generated by [DNDocs](https://dndocs.com/)";

                // if (project.ProjectType == ProjectType.NugetOrg)
                // {
                //     
                // }
                // else throw new Exception("projecttype");
                ...
                var info = Newtonsoft.Json.JsonConvert.SerializeObject(new
                {
                    ...
                    projectType = ProjectType.NugetOrg,
                ...
                string appFooter =
                    ...
                    $"{info}" +
                    "</script>" +
                    $"<script type=\"text/javascript\" src=\"/public/dndocs-docfx-script.js\"></script>" +
                    "</div>" +
                    "";

                docfxManager.SetDocfxJson(appFooter, appTitle, null);
                ...
                using (var tempZipFolder = osapi.CreateTempDir())
                {
                    ...
                    {
                        logger.LogInformation("uploading to docs.dndocs. id: {0} pkgname: {1} pkgver: {2}", project.ProjectId, project.PackageName, project.PackageVersion);
                        ...
                        logger.LogInformation("completed uploading to docs.dndocs. id: {0} pkgname: {1} pkgver: {2}", project.ProjectId, project.PackageName, project.PackageVersion);
