    ...
    public class TxRepository : ITxRepository
    {
        ...
        public async Task<IEnumerable<long>> ScriptForSitemapGenerator()
        {
            ...
            sql =
                ...
                "DELETE FROM sitemap_project AS sm WHERE sm.public_html_id IN " + 
                "  (SELECT ph.id FROM public_html ph WHERE [path] LIKE '/sitemaps/%' AND length(byte_data) < 10000000) " +
                "  AND (SELECT COUNT(*) FROM public_html WHERE [path] LIKE '/sitemaps/%') > 5;" + // dont want to always del only few sitemaps, wait for little more
